...
class MultiCudaStreamScheduler(Scheduler):
    ...
    def get_final_events_to_sync(self) -> set[CudaEventSym]:
        ...
        if self.unjoined_events.get(DEFAULT_STREAM_IDX):
            raise ValueError(
                f"Unexpected self.unjoined_events[DEFAULT_STREAM_IDX]={self.unjoined_events[DEFAULT_STREAM_IDX]} on default stream",
            )
        events_to_sync = set()
        for stream, events in self.unjoined_events.items():
            ...
            if 1 < len(events):
                schedule_log.debug(
                    f"Seeing multiple hanging events={events} on stream{stream}, scheduling the "
                    f"last_event={last_event} to sync",
                )
            else:
                schedule_log.debug(
                    f"Scheduling the last_event={last_event} on stream{stream} to sync",
                )
        ...
        for node in self.nodes:
            ...
            if node.is_template():
                ...
                if isinstance(backend_, (SIMDScheduling, CUDACombinedScheduling)):
                    ...
                    raise AssertionError(f"type(self)={type(self)}")
